...
class sugar_fitting:
    ...
    def __init__(self, x, y, covx, covy,
                 ...
                 fit_grey=False, fit_Av=False,
                 sparse=False, control=False, Rv=2.6):
        self.fit_grey = fit_grey
        self.fit_Av = fit_Av
        self.Rv = Rv
        #if fit_grey and fit_Av:
        #    raise ValueError("Can not fit for the momment Av and grey offset at the same time.")
        ...
        self.color = 1 if self.fit_Av else 0
        ...
        if self.fit_Av:
            self.filter_grey[1+self.grey] = False
        ...
        self.gamma_lambda = sugar.extinctionLaw(self.wavelength, Rv=self.Rv)
    ...
    def init_fit(self, alpha=None, M0=None, delta_m_grey=None, a_lambda0=None, gamma_lambda=None):
        ...
        if a_lambda0 is not None:
            if not self.fit_Av:
                raise ValueError("should set the init with option fit_Av=True")
            else:
    ...
    def e_step(self):
        ...
        and the a_lambda0 (if fit_Av is True).
        """
    ...
    def decorrelate_grey_h(self):
        self.separate_component()
        if self.fit_Av:
            h = np.zeros((self.nsn, len(self._h[0]) + 1 ))
            for ncomp in range(len(self._h[0])):
                h[:,ncomp] = self._h[:,ncomp]
            h[:,-1] = self.a_lambda0
        else:
            h = copy.deepcopy(self._h)

        self.add_cst_xplus = np.zeros(len(h[0]))
        self.cov_grey_xplus = np.zeros(len(h[0]))
        ...
        if self.fit_Av:
            self.delta_m_grey -= h[:,-1]*self.add_cst_xplus[-1]
            self.gamma_lambda += self.add_cst_xplus[-1]
    ...
    def _m_step(self):
        ...
        if self.fit_Av:
            Y -= (self.h[:,1+self.grey].reshape((self.nsn, 1))) * (np.ones(np.shape(self.y)) * self.A[:,1+self.grey])
    ...
    def run_fit(self, maxiter=10000, not_build_disp=False):
    ...
    def merge_component(self):
        ...
        if self.fit_Av:
            ...
            self.A[:,self.grey+self.color] = copy.deepcopy(self.gamma_lambda)
